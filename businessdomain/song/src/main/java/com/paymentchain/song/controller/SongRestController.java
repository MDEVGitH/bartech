package com.paymentchain.song.controller;

import com.paymentchain.song.entities.Song;
import com.paymentchain.song.respository.SongRepository;
...
public class SongRestController {
    ...
    @GetMapping("/notplayed")
    public ResponseEntity<?> listNotPlayed() {
        List<Song> songList = songRepository.findNotPlayed();
        if(songList.isEmpty()) {
            return ResponseEntity.noContent().build();
        }
        return ResponseEntity.ok().body(songList);
